...
for shop in range(1, 3001):
    ...
    list_of_values = list(range(int(sale_i_df['sale_amt'].min()), int(sale_i_df['sale_amt'].max()) + 1, 10))
    ...
    for pre_sale in list_of_values:
        ...
        sale_i_df['decay'] = list(91-(np.arange(1, len_of_sale_i + 1, 1))**2)
        #print(sale_i_df)
        loss_May.append((sale_i_df['sale_amt_diff'] / sale_i_df['decay']).sum())
        loss_June.append((sale_i_df['sale_amt_diff'] / (sale_i_df['decay'] + 10)).sum())
        loss_July.append((sale_i_df['sale_amt_diff'] / (sale_i_df['decay'] + 24)).sum())
...
sub.to_csv(r'./submission_20171126001.csv', index=None, header=None)
