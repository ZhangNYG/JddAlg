...
'''店铺的广告信息'''
# t_ads = pd.read_csv(r'./t_ads.csv', low_memory=False)
'''店铺的评论信息'''
# t_comment = pd.read_csv(r'./t_comment.csv', low_memory=False)
'''店铺的订单信息'''
t_order = pd.read_csv(r'./t_order.csv', low_memory=False)
'''店铺的商品信息'''
# t_product = pd.read_csv(r'./t_product.csv', low_memory=False)
'''店铺每月末后90天的销售额'''
...
def get_feature_data(dataset):
    ...
    #print(feature_2,feature_3,feature_4,feature_5)
    '''offer_amt'''
...
#38     116237.17
    ...
    #print("train_feature_data_______________________end",train_feature_data)
    #print("label_data______________________________end",label_data)
    module = GradientBoostingRegressor(n_estimators=200, learning_rate=0.1, max_depth=6)
    print(train_feature_data.head(10))
...
print(values)
values.to_csv('result20171122001.csv', index=None, header=None)
print('Finished!')
